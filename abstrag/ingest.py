...
from abstrag.utils import get_user_agent
...
def retrieve_arxiv_metadata(
    ...
    """
    # Open arXiv client
    # Increased delay to avoid rate limiting (arXiv recommends 3 seconds between requests)
    # Using 4 seconds to be safe when fetching multiple pages
    client = arxiv.Client(
        page_size=100,
        delay_seconds=4.0,
        num_retries=3,
...
def paper_html_to_markdown(paper_id: PaperID, verbose: bool = False) -> str | None:
    ...
    # First check if HTML version exists with HEAD request (faster, saves bandwidth)
    try:
        head_response = requests.head(url_html, headers=headers, allow_redirects=True, timeout=10)
        if head_response.status_code != 200:
            if verbose:
                logger.debug(
                    f"HTML version not available for {paper_id['id']}. Status code: {head_response.status_code}"
                )
            return None
    except requests.RequestException as e:
        # If HEAD request fails, try GET anyway (some servers don't support HEAD properly)
        if verbose:
            logger.debug(f"HEAD request failed for {paper_id['id']}, trying GET: {e}")
    
    # Fetch the full HTML
    try:
        results = requests.get(url_html, headers=headers, timeout=30)
    except requests.RequestException as e:
        logger.warning(f"Request failed for {url_html}: {e}")
        return None
